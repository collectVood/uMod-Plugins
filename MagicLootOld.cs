...
{
    [Info("Magic Loot Old", "Norn", "0.1.26")]
    [Description("Simple components multiplier and loot system")]
    class MagicLootOld : RustPlugin
    {
        ...
        void SaveMagicLootData() { Interface.Oxide.DataFileSystem.WriteObject(this.Name, LootData); }
        void LoadMagicLootData()
        {
            int newitems = 0;
            LootData = Interface.Oxide.DataFileSystem.ReadObject<MLData>(this.Name);
            ...
            if (newitems != 0) { Puts("Added " + newitems.ToString() + " new items to /data/" + this.Name + ".json"); SaveMagicLootData(); }
			
			// RCon Logging
            Puts("Loaded " + LootData.ExtraLootList.Count + " item limits from /data/" + this.Name + ".json");
            Puts("Loaded " + Components.list.Count + " components from /config/" + this.Name + ".json");
        }
